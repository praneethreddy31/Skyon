...
import { Listing, Store, DailyDish } from '../types';
...
const Home: React.FC = () => {
  ...
  const [latestDishes, setLatestDishes] = useState<DailyDish[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [listings, stores, dishes] = await Promise.all([
          getCollection<Listing>('listings'),
  ...
  if (loading) {
    return <div className="text-center py-10">Loading community dashboard...</div>;
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Link to="/marketplace" className="block group">
            <Card className="h-full p-6 flex flex-col justify-between transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl">
                ...
                ) : <p className="mt-4 pt-4 border-t text-sm text-gray-400">No items in the market yet.</p>}
            </Card>
        </Link>
        ...
        <Link to="/bazaar" className="block group">
             <Card className="h-full p-6 flex flex-col justify-between transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl">
                ...
                ): <p className="mt-4 pt-4 border-t text-sm text-gray-400">No stores in the bazaar yet.</p>}
            </Card>
        </Link>
      </div>
      ...
      <Link to="/todays-dishes" className="block group">
        <Card className="transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl">
          <div className="p-6">
            ...
            ) : <p className="mt-4 pt-4 border-t text-sm text-gray-400">No dishes posted today.</p>}
          </div>
        </Card>
      </Link>
